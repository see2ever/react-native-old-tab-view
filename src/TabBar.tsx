...
export default class TabBar<T extends Route> extends React.Component<Props<T>, State> {
  ...
  private getMemoizedTabWidthGettter = memoize(
    (
      ...
      (i: number) =>
        this.getComputedTabWidth(i, layout, routes, scrollEnabled, tabWidths, flattenedWidth),
  );
  ...
  private getTranslateX = memoize((scrollAmount: Animated.Node<number>, maxScrollDistance: number) =>
    Animated.multiply(
      Platform.OS === 'android' && I18nManager.isRTL ? Animated.sub(maxScrollDistance, scrollAmount) : scrollAmount,
  ...
  render() {
    ...
    const getTabWidth = this.getMemoizedTabWidthGettter(
      layout,
      routes,
      scrollEnabled,
      tabWidths,
    ...
    const labelSumWidth = routes.reduce((m, _, i) => m + getTabWidth(i), 0);
    const blankWidth = layout.width - labelSumWidth;
