...
import { Layout, NavigationState, Route, Listener, PagerCommonProps, EventEmitterProps } from './types';
...
export default class Pager<T extends Route> extends React.Component<Props<T>, ComponentState> {
  ...
  componentDidUpdate(prevProps: Props<T>) {
    const { navigationState, layout, swipeVelocityImpact, springVelocityScale, springConfig, timingConfig } =
      this.props;
    ...
    if (prevProps.swipeVelocityImpact !== swipeVelocityImpact) {
      this.swipeVelocityImpact.setValue(
        swipeVelocityImpact !== undefined ? swipeVelocityImpact : SWIPE_VELOCITY_IMPACT,
      );
    }

    if (prevProps.springVelocityScale !== springVelocityScale) {
      this.springVelocityScale.setValue(
        springVelocityScale !== undefined ? springVelocityScale : SPRING_VELOCITY_SCALE,
      );
    }

    if (prevProps.springConfig !== springConfig) {
      this.springConfig.damping.setValue(
        springConfig.damping !== undefined ? springConfig.damping : SPRING_CONFIG.damping,
      );
      ...
      this.springConfig.stiffness.setValue(
        springConfig.stiffness !== undefined ? springConfig.stiffness : SPRING_CONFIG.stiffness,
      );
    ...
    if (prevProps.timingConfig !== timingConfig) {
      this.timingConfig.duration.setValue(
        timingConfig.duration !== undefined ? timingConfig.duration : TIMING_CONFIG.duration,
      );
    }
  }
  ...
  private indexAtSwipeEnd: Animated.Value<number> = new Value(this.props.navigationState.index);
  ...
  private swipeVelocityImpact = new Value(
    this.props.swipeVelocityImpact !== undefined ? this.props.swipeVelocityImpact : SWIPE_VELOCITY_IMPACT,
  );

  private springVelocityScale = new Value(
    this.props.springVelocityScale !== undefined ? this.props.springVelocityScale : SPRING_VELOCITY_SCALE,
  );
  ...
  private springConfig = {
    damping: new Value(
      this.props.springConfig.damping !== undefined ? this.props.springConfig.damping : SPRING_CONFIG.damping,
    ),
    mass: new Value(this.props.springConfig.mass !== undefined ? this.props.springConfig.mass : SPRING_CONFIG.mass),
    stiffness: new Value(
      this.props.springConfig.stiffness !== undefined ? this.props.springConfig.stiffness : SPRING_CONFIG.stiffness,
    ),
  ...
  private timingConfig = {
    duration: new Value(
      this.props.timingConfig.duration !== undefined ? this.props.timingConfig.duration : TIMING_CONFIG.duration,
    ),
  };
      ...
      cond(
        ...
        [
          cond(
            ...
            I18nManager.isRTL
              ? set(this.initialVelocityForSpring, multiply(-1, this.velocityX, this.springVelocityScale))
        ...
        timing(this.clock, { ...state, frameTime }, { ...TIMING_CONFIG, ...this.timingConfig, toValue }),
      ),
  ...
  private extrapolatedPosition = add(this.gestureX, multiply(this.velocityX, this.swipeVelocityImpact));
  ...
  private maybeCancel = block([
    cond(
      and(
        ...
        or(
          and(eq(this.index, sub(this.routesLength, 1)), lessThan(this.gestureX, 0)),
  ...
  private translateX = block([
    ...
    onChange(
      ...
      cond(
        I18nManager.isRTL ? lessThan(this.gestureX, 0) : greaterThan(this.gestureX, 0),
  ...
  private getTranslateX = memoize(
    (layoutWidth: Animated.Node<number>, routesLength: Animated.Node<number>, translateX: Animated.Node<number>) =>
      multiply(
        // Make sure that the translation doesn't exceed the bounds to prevent overscrolling
        min(max(multiply(layoutWidth, sub(routesLength, 1), DIRECTION_RIGHT), translateX), 0),
      ...
      render: children => (
        <PanGestureHandler
          ...
          <Animated.View
            ...
            <PagerContext.Provider value={this.providerVal}>{children}</PagerContext.Provider>
          </Animated.View>
        </PanGestureHandler>
      ),
    });
