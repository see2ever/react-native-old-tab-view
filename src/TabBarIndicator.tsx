...
export default class TabBarIndicator<T extends Route> extends React.Component<Props<T>> {
  ...
  private getTranslateX = memoize((position: Animated.Node<number>, routes: Route[], getTabWidth: GetTabWidth) => {
    ...
    // 只针对 justify-content: space-around
    const gapWidth = this.props.blankWidth / routes.length;
    const endsWidth = gapWidth / 2;
    ...
    const outputRange = routes.reduce<number[]>((acc, _, i) => {
      ...
      return [...acc, acc[i - 1] + getTabWidth(i - 1) + gapWidth];
    }, []);
